"""
WikiPathways database adapter for PathwayLens.
"""
...
class WikiPathwaysAdapter(BaseAdapter):
    """Adapter for WikiPathways database."""
    
    def __init__(self, base_url: str = "https://wikipathways.org", rate_limit: float = 5.0):
        """
        Initialize WikiPathways adapter.
        
        Args:
            base_url: Base URL for WikiPathways API
            rate_limit: Rate limit in requests per second
        """
        super().__init__(base_url, rate_limit)
        self.logger = logger.bind(module="wikipathways_adapter")
        
        # WikiPathways species codes
        self.species_codes = {
    ...
    async def get_pathways(self, species: SpeciesType) -> List[PathwayInfo]:
        ...
        if not species_name:
            self.logger.warning(f"Species {species} not supported by WikiPathways")
            return []
        
        try:
            # Get pathway list
            response = await self._make_request(f"wpi/webservice/webservice.php?func=listPathways&organism={species_name}")
            ...
            for pathway_data in response:
                pathways.append(PathwayInfo(
                    ...
                    url=f"https://wikipathways.org/pathways/{pathway_data.get('id', '')}"
                ))
    ...
    async def get_pathway_genes(self, pathway_id: str, species: SpeciesType) -> List[str]:
        ...
        try:
            response = await self._make_request(f"wpi/webservice/webservice.php?func=getPathway&pwId={pathway_id}")
            
            genes = []
            for gene_data in response.get('genes', []):
    ...
    async def get_gene_pathways(self, gene_id: str, species: SpeciesType) -> List[str]:
        ...
        try:
            response = await self._make_request(f"wpi/webservice/webservice.php?func=getPathwaysByXref&ids={gene_id}")
    ...
    async def get_gene_info(self, gene_id: str, species: SpeciesType) -> Optional[GeneInfo]:
        ...
        try:
            response = await self._make_request(f"wpi/webservice/webservice.php?func=getXrefList&ids={gene_id}")
            ...
            return GeneInfo(
                ...
                url=f"https://wikipathways.org/gene/{gene_id}"
            )
    ...
    async def convert_gene_id(self, gene_id: str, from_type: IDType, to_type: IDType, species: SpeciesType) -> Optional[str]:
        ...
        # WikiPathways primarily uses its own gene IDs
        # This is a simplified implementation
        if from_type == to_type:
            return gene_id
        
        # For now, return the same ID
        # In a full implementation, this would use WikiPathways' conversion services
        return gene_id
    ...
    async def get_database_info(self) -> Dict[str, Any]:
        ...
        return {
            "name": "WikiPathways",
            "description": "WikiPathways Database",
            "version": "Latest",
